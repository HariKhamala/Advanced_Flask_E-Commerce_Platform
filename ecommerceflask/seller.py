...
bp = Blueprint('seller', __name__, url_prefix='/seller')  # Set url_prefix='/seller'
...
@bp.route("/", methods=["GET", "POST"])  # This is now the root route for /seller
def seller():
    if request.method == 'POST':
        if 'bimage' not in request.files:
            flash('Couldn\'t find file part')
            return redirect(url_for('seller.seller'))  # Corrected URL
